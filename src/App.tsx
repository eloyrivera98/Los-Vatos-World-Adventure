...
type Pin={id:string;number:number;city:string;country:string;lng:number;lat:number;count:number;author:string;authorAvatar?:string;date:string;story?:string;message?:string;photo?:string;color:string;owned:boolean;collected:boolean;discoveries:Discovery[]}
...
function ScanFlow({onClose}:{onClose:()=>void;onCollect:(id:string)=>void}) {
  const [mode,setMode]=useState<'choose'|'activate'|'working'|'success'|'error'>('choose');const [title,setTitle]=useState('');const [city,setCity]=useState('');const [country,setCountry]=useState('');const [story,setStory]=useState('');const [message,setMessage]=useState('');const [photo,setPhoto]=useState('');const [error,setError]=useState('');const [result,setResult]=useState<any>(null)
