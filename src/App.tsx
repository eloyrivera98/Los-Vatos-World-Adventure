...
function CollectibleCard({pin,locked=false,reveal=false}:{pin:Pin;locked?:boolean;reveal?:boolean}) {
  ...
  return <><article className={'collectible-card '+(locked?'is-locked ':'')+(reveal?'card-reveal':'')} style={{'--card-color':pin.color} as React.CSSProperties}>
    ...
    <div className={'card-photo '+(canOpenPhoto?'can-enlarge':'')}>{!locked?(pin.photo?<img src={pin.photo} alt={`Recuerdo de ${pin.city}`} role="button" tabIndex={0} onClick={openPhoto} onKeyDown={event=>(event.key==='Enter'||event.key===' ')&&openPhoto(event)}/>:<div className="photo-empty"><Camera/><span>Sin fotografía</span></div>):pin.hintUnlocked&&pin.photo?<><img src={pin.photo} alt={`Pista para encontrar el cromo de ${pin.city}`} role="button" tabIndex={0} onClick={openPhoto} onKeyDown={event=>(event.key==='Enter'||event.key===' ')&&openPhoto(event)}/><span className="hint-ribbon">Pista desbloqueada</span></>:<div className="locked-art"><LockKeyhole/><span>Sin descubrir</span></div>}<span className="card-place">{pin.title||pin.city}</span>{canOpenPhoto&&<span className="enlarge-photo-hint"><ZoomIn/> Ampliar</span>}</div>
