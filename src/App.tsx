...
import {
  ...
  ShieldCheck, Sparkles, Trophy, UserRound, Users, X, Zap, ZoomIn, ZoomOut
} from 'lucide-react'
...
const logoutToHome=async()=>{sessionStorage.removeItem('lvwa-access');sessionStorage.removeItem('lvwa-after-auth');try{await authClient.signOut()}finally{window.location.replace('/')}}
...
function CollectibleCard({pin,locked=false,reveal=false}:{pin:Pin;locked?:boolean;reveal?:boolean}) {
  ...
  const canOpenPhoto=Boolean(pin.photo&&(!locked||pin.hintUnlocked))
  const openPhoto=(event:React.MouseEvent|React.KeyboardEvent)=>{event.stopPropagation();if(canOpenPhoto)setPhotoOpen(true)}
  return <><article className={'collectible-card '+(locked?'is-locked ':'')+(reveal?'card-reveal':'')} style={{'--card-color':pin.color} as React.CSSProperties}>
    ...
    <div className={'card-photo '+(canOpenPhoto?'can-enlarge':'')} role={canOpenPhoto?'button':undefined} tabIndex={canOpenPhoto?0:undefined} onClick={openPhoto} onKeyDown={event=>canOpenPhoto&&(event.key==='Enter'||event.key===' ')&&openPhoto(event)}>{!locked?(pin.photo?<img src={pin.photo} alt={`Recuerdo de ${pin.city}`}/>:<div className="photo-empty"><Camera/><span>Sin fotografía</span></div>):pin.hintUnlocked&&pin.photo?<><img src={pin.photo} alt={`Pista para encontrar el cromo de ${pin.city}`}/><span className="hint-ribbon">Pista desbloqueada</span></>:<div className="locked-art"><LockKeyhole/><span>Sin descubrir</span></div>}<span className="card-place">{pin.title||pin.city}</span>{canOpenPhoto&&<span className="enlarge-photo-hint"><ZoomIn/> Ampliar</span>}</div>
  ...
  </article>{photoOpen&&pin.photo&&createPortal(<PhotoViewer src={pin.photo} title={pin.title||pin.city} onClose={()=>setPhotoOpen(false)}/>,document.body)}</>
}
function PhotoViewer({src,title,onClose}:{src:string;title:string;onClose:()=>void}){
  const [zoom,setZoom]=useState(1);const stage=useRef<HTMLDivElement>(null);const pointers=useRef(new Map<number,{x:number;y:number}>());const gesture=useRef({distance:0,zoom:1})
  const clamp=(value:number)=>Math.min(5,Math.max(1,Math.round(value*100)/100));const distance=()=>{const points=[...pointers.current.values()];return points.length<2?0:Math.hypot(points[0].x-points[1].x,points[0].y-points[1].y)}
  const pointerDown=(event:React.PointerEvent<HTMLDivElement>)=>{event.currentTarget.setPointerCapture(event.pointerId);pointers.current.set(event.pointerId,{x:event.clientX,y:event.clientY});if(pointers.current.size===2){gesture.current={distance:distance(),zoom}}}
  const pointerMove=(event:React.PointerEvent<HTMLDivElement>)=>{const previous=pointers.current.get(event.pointerId);if(!previous)return;pointers.current.set(event.pointerId,{x:event.clientX,y:event.clientY});if(pointers.current.size>=2&&gesture.current.distance){event.preventDefault();setZoom(clamp(gesture.current.zoom*distance()/gesture.current.distance))}else if(pointers.current.size===1&&zoom>1&&stage.current){event.preventDefault();stage.current.scrollLeft-=event.clientX-previous.x;stage.current.scrollTop-=event.clientY-previous.y}}
  const pointerUp=(event:React.PointerEvent<HTMLDivElement>)=>{pointers.current.delete(event.pointerId);if(pointers.current.size<2)gesture.current.distance=0}
  useEffect(()=>{const key=(event:KeyboardEvent)=>{if(event.key==='Escape')onClose();if(event.key==='+')setZoom(value=>clamp(value+.5));if(event.key==='-')setZoom(value=>clamp(value-.5))};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key)},[onClose])
  return <div className="photo-viewer" role="dialog" aria-modal="true" aria-label={`Fotografía ampliada: ${title}`} onMouseDown={event=>{event.stopPropagation();if(event.target===event.currentTarget)onClose()}} onClick={event=>event.stopPropagation()}><header><div><small>FOTO-PISTA</small><b>{title}</b></div><button onClick={onClose} aria-label="Cerrar fotografía"><X/></button></header><div ref={stage} className="photo-zoom-stage" onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onDoubleClick={()=>setZoom(value=>value>1?1:2.5)} onWheel={event=>{event.preventDefault();setZoom(value=>clamp(value+(event.deltaY<0?.25:-.25)))}}><img src={src} alt={title} draggable={false} style={{width:`${zoom*100}%`}}/></div><footer><button onClick={()=>setZoom(value=>clamp(value-.5))} disabled={zoom<=1}><ZoomOut/></button><input type="range" min="1" max="5" step="0.1" value={zoom} onChange={event=>setZoom(Number(event.target.value))} aria-label="Nivel de ampliación"/><span>{Math.round(zoom*100)}%</span><button onClick={()=>setZoom(value=>clamp(value+.5))} disabled={zoom>=5}><ZoomIn/></button></footer><p>Pellizca con dos dedos para ampliar y arrastra para recorrer la pista.</p></div>
}
